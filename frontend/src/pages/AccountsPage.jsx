...
import { LuBell, LuCircleAlert, LuCircleCheck, LuClipboard, LuFileText, LuSearch, LuX } from "react-icons/lu";
...
export default function AccountsPage() {
    ...
    if (loading) return <LoadingModal message="Loading NAEFs..." subtext="Please wait while we fetch your data." />;
      ...
      ${successMessage ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
                        ...
                        <div className="flex space-x-3">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="space-y-1">
                                    <p className="text-sm text-gray-900">{newAssignedAccounts.map((tr) => tr.trNumber).join(", ")}</p>
                                </div>
                                <div className="mt-3">
                                    <button
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="space-y-1">
                                    <p className="text-sm font-medium text-gray-900">
                                        {newAssignedAccounts[0].trNumber} - {newAssignedAccounts[0].title}
                                    </p>
                                    <p className="text-sm text-gray-600">Account: {newAssignedAccounts[0].accountId}</p>
                                    <p className="text-sm text-gray-600">Contact: {newAssignedAccounts[0].contactPerson}</p>
                                </div>
                                <div className="mt-3">
                                    <button
                                        onClick={() => {
        ...
        <div
            className={`absolute overflow-auto top-0 right-0 h-full w-full bg-white shadow-xl transition-all duration-300 ${
                selectedAccount && !editingAccount ? "translate-x-0 opacity-100" : "translate-x-full opacity-0"
            }`}>
            {selectedAccount && !editingAccount && (
                <AccountDetails
        ...
        <div
            className={`absolute top-0 right-0 h-full w-full bg-white shadow-xl transition-all duration-300 ${
                editingAccount ? "translate-x-0 opacity-100" : "translate-x-full opacity-0"
            }`}>
            {editingAccount && (
                <AccountForm
