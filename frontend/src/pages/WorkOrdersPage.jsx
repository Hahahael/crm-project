...
export default function WorkOrdersPage() {
  ...
  const fetchAssignedWorkOrders = async () => {
    try {
      const res = await apiBackendFetch("/api/workorders/assigned");
      if (res.ok) {
  ...
  const newAssignedWorkOrders = currentUser
  ? workOrders.filter(
    ...
    <div className="relative w-full h-full overflow-hidden bg-white">
      ...
      {!selectedWO && !editingWO && (
        <div className="transition-all duration-300 h-full w-full p-6 overflow-y-auto">
          ...
          {currentUser && assignedWorkOrders.length > 1 && (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <p className="text-sm font-semibold text-orange-800">
                      {`You have ${assignedWorkOrders.length} new work order${assignedWorkOrders.length > 1 ? "s" : ""} assigned to you`}
                    </p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-sm text-gray-900">
                      {assignedWorkOrders.map(wo => wo.woNumber).join(", ")}
                    </p>
                  </div>
                  <div className="mt-3">
                    <button
                      onClick={() => setSelectedWO(assignedWorkOrders[0])}
            ...
            <div className="flex border-orange-200 border-2 rounded-xl p-4 mb-6 bg-orange-50 items-start justify-between text-card-foreground shadow-lg animate-pulse">
              <div className="flex space-x-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm font-medium text-gray-900">{assignedWorkOrders[0].woNumber} - {assignedWorkOrders[0].workDescription}</p>
                    <p className="text-sm text-gray-600">Account: {assignedWorkOrders[0].accountName}</p>
                    <p className="text-sm text-gray-600">Contact: {assignedWorkOrders[0].contactPerson}</p>
                  </div>
                  <div className="mt-3">
                    <button
