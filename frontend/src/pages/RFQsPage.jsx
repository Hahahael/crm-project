...
export default function RFQsPage() {
    ...
    const handleSave = async (formData, mode) => {
        ...
        try {
            ...
            // 5. Workflow stage logic (unchanged)
            if (mode === "edit") {
                // Fetch the workflow stage for this workorder and stage name
                const wsRes = await apiBackendFetch(`/api/workflowstages/workorder/${rfqId}`);
                if (wsRes.ok) {
                    const stages = await wsRes.json();
                    // Find the "RFQ" stage
                    const rfqStage = stages.find((s) => s.stage_name === "RFQ");
                    if (rfqStage) {
                        await apiBackendFetch(`/api/workflowstages/${rfqStage.woId}`, {
                            method: "PUT",
                            body: JSON.stringify({
                                status: rfqData.status || "Pending",
                                assigned_to: rfqData.assignee,
                            }),
                        });
                    }
                }
