...
import { LuEllipsis, LuEye, LuPencil, LuTrash, LuCheck, LuClock, LuCircleAlert } from "react-icons/lu";
        ...
        <tbody className="divide-y divide-gray-200">
          {rfqs.map((rfq) => (
            <tr key={rfq.id} className="hover:bg-gray-50 transition-all duration-200">
              ...
              <td className="px-4 py-2 text-black text-sm">{util.formatDate(rfq.createdAt, "DD/MM/YYYY")}</td>
              ...
              <td className="px-4 py-2 text-black text-sm">{renderStatusBadge(rfq.stageStatus)}</td>
              ...
              <td className="px-4 py-2 text-black text-sm">
                {(() => {
                  const { status, daysLate } = util.calculateTimeliness(rfq.dueDate, rfq.doneDate);
