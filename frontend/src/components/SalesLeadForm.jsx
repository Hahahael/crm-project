...
const SalesLeadForm = ({ salesLead, mode = "create", onSave, onBack }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const { slNumber, immediateSupport, existingSpecifications, quantityAttention, qrCc, doneDate, salesPlanRep, ww, existingSetupItems, customerSuggested, remarks, actualPicture, draftDesignLayout, consideration, issuesWithExisting, customerIssues, customerSuggestedSetup, ...requiredFields } = formData;
    ...
    <form onSubmit={handleSubmit} className="h-full w-full p-6 overflow-y-auto">
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex flex-col">
          ...
          >
            <LuArrowLeft className="my-auto text-lg" />&nbsp;Back to Sales Lead Details
          </button>
          <h1 className="text-2xl font-bold">
            {mode === "edit" ? "Edit Sales Lead" : "New Sales Lead"}
          </h1>
          <h2 className="text-lg text-gray-500">
            {salesLead?.slNumber ? `${salesLead.slNumber}` : "FSL# (auto-generated)"}
          </h2>
        </div>
