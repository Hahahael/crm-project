...
const SalesLeadDetails = ({
  ...
  const [activeTab, setActiveTab] = useState("SL"); // SL | WO
  ...
  // Load related Work Order when switching to Work Order tab
  useEffect(() => {
    ...
    if (activeTab === "WO" && !woDetails) {
      fetchWorkOrder();
    }
  }, [activeTab, salesLead?.woId, salesLead?.wo_id, woDetails]);
