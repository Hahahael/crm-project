...
async function resolveCustomerId(pool, candidateId, candidateName) {
  try {
    ...
    if (candidateId != null) {
      ...
      const r = await req.query('SELECT Id FROM spidb.customer WHERE Id = @cid');
    ...
    if (candidateName) {
      ...
      const r2 = await req2.query("SELECT TOP (1) Id FROM spidb.customer WHERE [Name] LIKE @cname");
      if (r2 && r2.recordset && r2.recordset.length > 0) return r2.recordset[0].Id;
    }
    ...
    if (r3 && r3.recordset && r3.recordset.length > 0) return r3.recordset[0].Id;
    return null;
  } catch (err) {
...
router.get('/quotations/:id', async (req, res) => {
  try {
    ...
    const qRes = await pool.request().input('id', id).query('SELECT * FROM quotation WHERE id = @id');
    ...
    const detailsRes = await pool.request().input('quotation_id', id).query('SELECT * FROM quotation_details WHERE quotation_id = @quotation_id');
...
router.post('/quotations', async (req, res) => {
  ...
  if (!quotation || Object.keys(quotation).length === 0) {
    return res.status(400).json({ error: 'Quotation object is required in the request body' });
  }
