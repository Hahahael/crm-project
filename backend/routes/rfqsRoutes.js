...
router.get("/:id", async (req, res) => {
    try {
        ...
        const itemsRes = await db.query(`SELECT * FROM rfq_items WHERE rfq_id = $1 ORDER BY id ASC`, [id]);
        ...
        const vendorsRes = await db.query(`SELECT * FROM rfq_vendors WHERE rfq_id = $1 ORDER BY id ASC`, [id]);
    ...
    try {
        ...
        if (items && Array.isArray(items)) {
            ...
            for (const item of items) {
                ...
                let tempId = await db.query(`INSERT INTO rfq_items (rfq_id, item_id, quantity) VALUES ($1,$2,$3) RETURNING id`, [
                    newId,
                    item.item_id,
                    item.quantity,
                ]);
...
router.put("/:id", async (req, res) => {
    try {
        ...
        if (!actualFromTime) actualFromTime = new Date().toTimeString().split(' ')[0]; // HH:MM:SS
        ...
        if (body.vendors && Array.isArray(body.vendors)) {
            ...
            for (const vendor of body.vendors) {
                ...
                if (vendor.vendor_id && existingIds.has(vendor.vendor_id)) {
                    ...
                    await db.query(`INSERT INTO rfq_vendors (rfq_id, vendor_id, valid_until, payment_terms, notes) VALUES ($1,$2,$3,$4,$5)`, [
                        id,
                        vendor.vendor_id,
                        validUntil,
                        paymentTerms,
        ...
        if (allQuotations.length > 0) {
            ...
            for (const ex of existing) {
                if (!incomingIds.has(`${ex.vendor_id}-${ex.item_id}-${ex.rfq_id}`)) {
                    await db.query("DELETE FROM rfq_quotations WHERE item_id = $1 AND vendor_id = $2 AND rfq_id = $3", [
                        ex.item_id,
                        ex.vendor_id,
                        ex.rfq_id,
                    ]);
...
router.get("/:id/items", async (req, res) => {
    try {
        ...
        const itemsResult = await db.query(`SELECT * FROM rfq_items WHERE rfq_id = $1 ORDER BY id ASC`, [id]);
