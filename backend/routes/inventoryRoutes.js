...
router.get("/mssql/stocks/:id", async (req, res) => {
    try {
        ...
        const mergedDetails = details.map((d) => mergePrimaryWithParent(d, stock));
...
router.get("/mssql/stock-details/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const pool = await poolPromise;
        const resDetail = await pool.request().input("id", id).
        ...
        // fetch parent stock
        if (detail.stockId) {
            const stockId = detail.Stock_Id || detail.stockId || detail.StockId || detail.stock_id;

            const parentRes = await pool.request().input("id", stockId).
...
router.get("/mssql/vendors/:id", async (req, res) => {
    try {
        ...
        const detailsRes = await pool.request().input("vendor_id", id)
        ...
        const vendorRes = await pool.request().input("id", id)
