...
router.get('/mssql/stocks', async (req, res) => {
  try {
    ...
    const sql = 'SELECT TOP (@limit) * FROM spidb.stock_details ORDER BY Id DESC';
  ...
  logAttributes('stock_details (fetched)', details);
    ...
    const stockIds = [...new Set(details.map((d) => d.Stock_Id).filter(Boolean))];
    ...
    // merge: primary = detail, parent fields go into primary or as <field>_secondary when colliding
    const merged = details.map((d) => {
      const parent = stocksMap[d.Stock_Id || d.stockId || d.StockId || d.stock_id];
      if (!parent) return d;
      const out = { ...d };
      for (const k of Object.keys(parent)) {
        if (Object.prototype.hasOwnProperty.call(out, k) || Object.prototype.hasOwnProperty.call(out, k.toLowerCase())) {
...
router.get('/mssql/stocks/:id', async (req, res) => {
  ...
  const detailsRes = await pool.request().input('stock_id', id).query('SELECT * FROM spidb.stock_details WHERE Stock_Id = @stock_id');
  const details = detailsRes.recordset || [];
  logAttributes('stock_details for stock id ' + id, detailsRes.recordset || []);

    // Fetch parent stock
    const stockRes = await pool.request().input('id', id).query('SELECT * FROM spidb.stock WHERE Id = @id');
...
router.get('/mssql/stock-details/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const pool = await poolPromise;
  const resDetail = await pool.request().input('id', id).query('SELECT * FROM spidb.stock_details WHERE Id = @id');
  if (!resDetail.recordset || resDetail.recordset.length === 0) return res.status(404).json({ error: 'Not found' });
  const detail = resDetail.recordset[0];
  logAttributes('stock_detail (single) id ' + id, resDetail.recordset || []);
    // fetch parent stock
    if (detail.stockId) {
      const parentRes = await pool.request().input('id', detail.Stock_Id || detail.stockId || detail.StockId || detail.stock_id).query('SELECT * FROM spidb.stock WHERE Id = @id');
...
router.get('/mssql/vendors', async (req, res) => {
  try {
    ...
    const sql = 'SELECT TOP (@limit) * FROM spidb.vendor_details ORDER BY Id DESC';
  const vdRes = await request.query(sql);
  const details = vdRes.recordset || [];
  logAttributes('vendor_details (fetched)', vdRes.recordset || []);

    const vendorIds = [...new Set(details.map((d) => d.Vendor_Id).filter(Boolean))];
    let vendorsMap = {};
    ...
    const merged = details.map((d) => {
      const parent = vendorsMap[d.Vendor_Id || d.vendorId || d.vendor_id || d.VendorId];
      if (!parent) return d;
      const out = { ...d };
      for (const k of Object.keys(parent)) {
        if (Object.prototype.hasOwnProperty.call(out, k) || Object.prototype.hasOwnProperty.call(out, k.toLowerCase())) {
...
router.get('/mssql/vendors/:id', async (req, res) => {
  ...
  const detailsRes = await pool.request().input('vendor_id', id).query('SELECT * FROM spidb.vendor_details WHERE Vendor_Id = @vendor_id');
  const details = detailsRes.recordset || [];
  logAttributes('vendor_details for vendor id ' + id, detailsRes.recordset || []);

  const vendorRes = await pool.request().input('id', id).query('SELECT * FROM spidb.vendor WHERE Id = @id');
  if (!vendorRes.recordset || vendorRes.recordset.length === 0) return res.status(404).json({ error: 'Vendor not found' });
  const vendor = vendorRes.recordset[0];
  logAttributes('vendor (parent for id ' + id + ')', vendorRes.recordset || []);

    const mergedDetails = details.map((d) => {
      const out = { ...d };
      for (const k of Object.keys(vendor)) {
        if (Object.prototype.hasOwnProperty.call(out, k) || Object.prototype.hasOwnProperty.call(out, k.toLowerCase())) {
          out[`${k}_secondary`] = vendor[k];
        } else {
          out[k] = vendor[k];
        }
      }
